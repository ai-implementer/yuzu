...
v0.6 以降の候補（このリリースではやらない）: ドキュメントバージョニング（要否含め保留中）・フレーズ検索（トークナイザ据え置きで位置情報インデックスのフォーマット改版。Phase 28 で決定）・tantivy バックエンド（静的ホスティング方針と要調整）・i18n・VS Code 拡張（wasm プレビュー）・crates.io 公開とバイナリ配布・tankan の分離公開・yuzu 自身のドキュメントサイト公開・ビルドのページ並列化。
