...
v0.1（Phase 1〜6: build / dev サーバ / 日本語検索 / llms.txt / tankan SSR / fmt・lint・check）は完了。  
以下の Phase 7〜12 がすべて完了した時点で **v0.2** としてリリースする。  
Phase は価値と実装コスト・依存関係の順に並べている（着手時に個別に設計する）。
...
| **12 インクリメンタルビルド** | `build.incremental` ＋ `.yuzu/cache/`。dev / build --watch の再ビルドをページ単位に短縮 | ⬜ |
