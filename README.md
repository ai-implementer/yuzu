...
```jsonc
{
  ...
  "lint": { "maxDirectoryDepth": 1 }, // content 配下のディレクトリ階層を制限（直下 = 0。未設定なら無制限）
  "search": {
    "enabled": true,
...
| Phase | 内容 | 状態 |
|---|---|---|
| **13 執筆の即効改善** | draft プレビュー（`dev` / `build --drafts` で下書きページも確認）・Mermaid client 描画のダークモード切替時の再描画（既知の制限の解消）・テーマ調整の設定化（アクセントカラー・フォントを `yuzu.jsonc` から） | ⬜ |
| **14 ページ単位 .md 配信とページコピー** | 各ページの Markdown を `dist/<path>.md` に配信して llms.txt を `.md` リンク化（vitepress / docusaurus プラグインで優勢の形式）＋各ページに「Markdown をコピー」ボタン（配信した `.md` を fetch → クリップボード。コードコピーと同じプログレッシブエンハンスメント）。原文か正規化 md かの設計判断込み | ⬜ |
| **15 日本語 lint: 表記ゆれ・用語統一** | プロジェクト用語辞書による用語統一チェック＋組み込みルール（全角/半角・長音符ゆれ等）を `yuzu lint` / `check` に統合（行番号付き報告）。まず辞書ベースの最小版から | ⬜ |
| **16 tankan 図種追加** | 設計書頻出の class 図＋低コストの pie。corpus テスト・insta スナップショット・wasm32 担保は既存の図種追加手順に従う | ⬜ |
...
- ページ単位の md 配信（`dist/<path>.md` ＋ llms.txt の `.md` リンク化）は
  ロードマップ Phase 14 で対応予定
