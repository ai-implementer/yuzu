...
| Phase | 内容 | 状態 |
|---|---|---|
| **36 yuzu 自身のドキュメントサイト公開** | dogfooding の総仕上げとして、yuzu 自身のドキュメントを yuzu で書いて GitHub Pages に公開する。README の内容をページ階層（nav・TOC）へ再構成し、tankan SSR・OpenAPI レンダリング・検索（フレーズ・近接ブースト・OPFS キャッシュ）など主要機能を実際に使ったサイトとして見せる。このリポジトリ用の `yuzu.jsonc` ＋ content ツリー ＋ Pages デプロイ設定（`yuzu new` が生成する雛形とは別に、このリポジトリ自身の CI に組み込む）を用意する。crates.io 公開時の説明文からリンクできる実例サイトにもなる | ⬜ |
