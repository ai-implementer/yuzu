...
| Phase | 内容 | 状態 |
|---|---|---|
| **24 tankan スタイル構文の全図種展開** | flowchart で対応した `classDef` / `class` / `:::` / `style`（＋fill 明度からのラベル色自動選択）を **state / ER / class 図**へ展開。図種ごとの適用先（状態ボックス・エンティティ・クラスボックス）の設計込み | ⬜ |
| **25 検索: コードブロックの opt-in インデックス** | `search.indexCode` でフェンスコードブロックを検索対象に追加（既定 off）。関数名・設定キーで設計書を引ける。tf の重み・抜粋表示の扱いはインデックス影響を見て設計 | ⬜ |
| **26 OpenAPI レンダリングの拡充** | Swagger 2.0（`definitions` 等の旧レイアウト）対応と、`components/schemas` 一覧の描画（操作から参照されないスキーマも読める）。dogfooding の需要順で調整 | ⬜ |
| **27 tankan 新図種** | mindmap / timeline 等の未対応図種から 1〜2 図種を SSR 追加（クライアント描画フォールバックの利用頻度を見て選定）。I/O なし・時刻非依存の設計原則は維持 | ⬜ |
