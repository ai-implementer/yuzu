...
| **38 tankan の分離公開** | tankan は既に yuzu-* 非依存の設計（README「凍結した設計判断」参照）。crates.io 単独公開（`publish = false` を外しメタデータを整備して公開。将来の crates.io 公開順の先頭）に留めるか、独立 git リポジトリへの分離（コミット履歴の扱い・CI 複製・バージョニング独立化を伴う）まで行うかは着手時に判断する。Mermaid 互換 SSR だけを求める非 yuzu ユーザーへの訴求が目的 | ⬜ |

v0.8 以降の候補（このリリースではやらない）: ドキュメントバージョニング（要否含め保留中）・i18n（テーマ UI 文字列の多言語化。`site.lang` は既にあるが検索 UI・404 ページ等のテーマ文言は日本語ハードコード）・VS Code 拡張（wasm プレビュー）・crates.io 公開（tankan → 検索スタック（yuzu-index-format）→ yuzu 本体の順に切り離し公開。名前 `yuzu`・`yuzu-core` は別プロジェクトに取得済みのため、本体は内部 crate の単一パッケージ化と名称の検討が必要。Phase 37 の決定事項）。
