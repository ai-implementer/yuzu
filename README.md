...
- **日本語全文検索**（自前 BM25）: vaporetto の分かち書き＋**文字単位**の編集距離 1 の
  ...
  **`lint.terms` の用語辞書と `search.synonyms` がクエリ拡張に使われ、
  ...
  （既定 off。図・仕様・数式ソースの mermaid / openapi / jsonschema / math は除外）。
...
| **25 検索: コードブロックの opt-in インデックス** | `search.indexCode`（既定 off）でフェンスコードブロック本文を検索対象に追加。関数名・設定キーで設計書を引ける。tf 重みは本文と同じ 1・コードは抜粋にも出す（merge）・特別レンダリング対象（mermaid / openapi / jsonschema / math）は除外・llms.txt には非混入。envKey が on/off を拾いキャッシュ自動無効化 | ✅ |
