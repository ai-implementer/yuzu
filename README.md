...
v0.8 以降の候補（このリリースではやらない）: ドキュメントバージョニング（要否含め保留中）・i18n（テーマ UI 文字列の多言語化。`site.lang` は既にあるが検索 UI・404 ページ等のテーマ文言は日本語ハードコード）・VS Code 拡張（wasm プレビュー）・crates.io 公開の続き（tankan は v0.7 で公開済み。残りは検索スタック（yuzu-index-format）→ yuzu 本体の順。名前 `yuzu`・`yuzu-core` は別プロジェクトに取得済みのため、本体は内部 crate の単一パッケージ化と名称の検討が必要。Phase 37 の決定事項）。
