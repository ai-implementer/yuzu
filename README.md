...
| Phase | 内容 | 状態 |
|---|---|---|
| **42 コンテンツインクルード** | 実ソースファイルの一部をコードブロックへ埋め込む: ` ```rust file="src/api.rs" lines=10-25 ` の形で情報文字列に指定（Phase 39 の `parse_fence_info` 基盤に乗せる。openapi の `file:` は本文 1 行方式だが、コードは本文が空になるため情報文字列側に置く）。プロジェクトルート相対・ルート外拒否は apispec の canonicalize ガード（`ProjectSpecFiles`）を一般化して共用。`title` 省略時はファイル名を自動キャプションにし、行ハイライト / 行番号メタと合成可。参照ページは既存の external_deps 機構でキャッシュ非対象（参照先の変更が次ビルドで必ず反映）。ファイル不在・行範囲外はエラーボックスでビルド継続（apispec と同じ）+ `yuzu check` が検出。設計書とコードの乖離を防ぐのが目的。検索（indexCode）・llms への埋め込み内容の扱いと dev watch の参照先監視は着手時に判断 | ⬜ |
| **43 図表番号と相互参照** | 図（mermaid SSR の `<figure>`・コードブロックの `figure.code-block`・キャプション付き画像）と表に自動採番を付け、本文から「図 1」「表 2」で参照リンクできるようにする。記法は CommonMark からの逸脱を最小にする方向で着手時に設計（案: キャプション要素への id 付与 + 空テキストリンク `[](#fig:label)` の自動テキスト補完）。採番の単位（ページ内 / サイト全体）も着手時判断。参照切れは `yuzu check` が検出し、fmt / llms の整合を保つ | ⬜ |
| **44 折りたたみ** | Admonition の折りたたみ対応: `> [!NOTE]- タイトル` のように種別へ `-` を付けると `<details>/<summary>` で描画する（Obsidian callouts 互換の記法。ネイティブ要素なのでクライアント JS 不要）。既存 admonition のテーマ CSS と統合し、`yuzu fmt` はラウンドトリップ温存・折りたたみの中身も検索 / llms の本文として扱う | ⬜ |
| **45 dogfooding 改善** | 恒例のバッファ枠: 実運用で踏んだ不満を一括解消（着手時に個別選定）。OG メタ・favicon は方針どおり対象外 | ⬜ |
