...
v0.3（Phase 13〜18: 執筆の即効改善 / ページ Markdown 配信とコピー / 用語統一 lint / tankan class・pie / git 連携メタ / dogfooding 改善）は完了・リリース済み。

以下の Phase 19〜23 がすべて完了した時点で **v0.4** としてリリースする。  
実際の設計書運用（dogfooding）と並行して進め、Phase は価値と実装コスト・依存関係の順に並べている（着手時に個別に設計する）。

| Phase | 内容 | 状態 |
|---|---|---|
| **19 表記ゆれ lint の組み込みルール** | 辞書不要で検出できる機械的なゆれ（全角/半角の混在・長音符の有無・半角カナ等）を `term-variant` と並ぶ組み込みルールとして追加。ルール単位で有効化を選べる形にする | ⬜ |
| **20 検索の用語ゆれ・同義語対応** | `lint.terms` の辞書（＋任意の `search.synonyms`）をクエリ拡張に使い、ゆれ表記で検索しても正表記の文書がヒットするようにする。実装は yuzu-index-format に 1 箇所（native/wasm 共有の整合制約に沿う） | ⬜ |
| **21 検索 UX の磨き込み** | 結果件数表示・ヒットなし時の挙動・キーボード操作の細部・長い結果リストの扱い等。実戦で気になる細部を仕様化して潰す | ⬜ |
| **22 OpenAPI / JSON Schema レンダリング** | API 仕様の整形表示（埋め込み記法・SSR 自前か vendored viewer かの設計判断込み）。規模大のため着手時に個別設計 | ⬜ |
| **23 dogfooding 改善** | 実運用で溜まった不満の一括解消（バッファ枠）。v0.4 の締めとして実施 | ⬜ |

v0.5 以降の候補（このリリースではやらない）: ドキュメントバージョニング（要否含め別途検討）・検索辞書の高精度化（vibrato / lindera。トークナイザ整合制約で index/query 両側同時差し替えが必要）・tantivy バックエンド（静的ホスティング方針と要調整）・i18n・VS Code 拡張（wasm プレビュー）・crates.io 公開とバイナリ配布・tankan の分離公開・yuzu 自身のドキュメントサイト公開。
