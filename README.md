...
以下の Phase 36〜38 がすべて完了した時点で **v0.7** としてリリースする。軸は「公開・配布の整備」。  
...
| **37 crates.io 公開とバイナリ配布** | ワークスペース各クレートを依存順（yuzu-core → yuzu-config → tankan → yuzu-index-format → yuzu-index/yuzu-render/yuzu-search-wasm → yuzu-server → yuzu-cli）に crates.io 公開する。`description` は既に全クレートに設定済み（確認済み）。残作業: 各クレートのパッケージサイズ実測（`cargo package --list` / `--dry-run`。vaporetto モデル・mermaid/KaTeX 同梱アセット・rust-embed テーマ資産が crates.io の 10MB/crate 制限に収まるか）、README 未整備クレート（tankan・yuzu-index-format・yuzu-search-wasm 以外）への `readme` 追加要否の判断、`publish = false` にすべきクレート（yuzu-theme 等アセット専用か）の判断。バイナリ配布は tag push トリガーの GitHub Actions で主要プラットフォーム（macOS arm64/x64・Linux x64・Windows x64）向け `yuzu-cli` をビルドし GitHub Release へ添付（ツール選定は着手時判断） | ⬜ |
| **38 tankan の分離公開** | tankan は既に yuzu-* 非依存の設計（README「凍結した設計判断」参照）。Phase 37 で確立した公開パイプラインに乗せて crates.io 単独公開に留めるか、独立 git リポジトリへの分離（コミット履歴の扱い・CI 複製・バージョニング独立化を伴う）まで行うかは着手時に判断する。Mermaid 互換 SSR だけを求める非 yuzu ユーザーへの訴求が目的 | ⬜ |

v0.8 以降の候補（このリリースではやらない）: ドキュメントバージョニング（要否含め保留中）・i18n（テーマ UI 文字列の多言語化。`site.lang` は既にあるが検索 UI・404 ページ等のテーマ文言は日本語ハードコード）・VS Code 拡張（wasm プレビュー）。
