...
- **Mermaid**: ` ```mermaid ` ブロック → 既定は同梱 mermaid.js でクライアント描画。
  **`backend: "ssr"` にすると自作の [tankan](crates/tankan/) がビルド時に SVG 化**
  ...
  CSS 変数経由でダークモードに**再描画なしで追従**。flowchart は
  `classDef`（`default` 含む）/ `class` 文 / `:::` / `style` 文のスタイル指定も SSR — ユーザ指定色は
  意図どおりの固定色で描く。linkStyle / click はフォールバック）
- **API 仕様（OpenAPI / JSON Schema）**: ` ```openapi ` / ` ```jsonschema ` ブロックを
  **ビルド時に静的 HTML 化**（SSR 自前・クライアント JS ゼロ・テーマ/ダークモード統合）。
  ...
  解決する（循環は参照名表示）。パース失敗や Swagger 2.0 は
  エラーボックス表示でビルドは継続する
...
- **日本語全文検索**（自前 BM25）: vaporetto の分かち書き＋**文字単位**の編集距離 1 の
  ...
  **`lint.terms` の用語辞書と `search.synonyms` がクエリ拡張に使われ、
  ...
  （ハイライトも正表記側に乗る）。`yuzu search <クエリ>` で同じエンジンを
  ターミナルからも使える
...
| **23 dogfooding 改善** | 積み残しの一括解消（バッファ枠）: tankan flowchart のスタイル構文 SSR（`classDef` / `class` / `:::` / `style`）・OpenAPI のプロジェクト内ファイル間 `$ref` 解決（参照元ファイル相対・ルート外拒否・参照ページはキャッシュ非対象）・小粒の磨き込み（trace メソッド・description 二重表示修正・ドキュメント陳腐化） | ✅ |
