...
v0.9（Phase 42〜45: 執筆機能の拡充 第 2 弾＝コンテンツインクルード（`file=` で実ソースの行範囲を埋め込み） / 図表番号と相互参照（キャプション行で自動採番・空リンクを「図 1」へ補完） / 折りたたみ（`> [!NOTE]-` で `<details>` 化） / dogfooding 改善＝折りたたみの自動展開・fmt の独自記法温存・図表番号のサイト全体通し番号）は完了・リリース済み。v0.9.1 でサイドバーのスクロール位置をページ遷移をまたいで維持する改善を追加。

以下の Phase 46〜49 がすべて完了した時点で **v0.10** としてリリースする。軸は「実運用の質を上げる」。  
これまでの「書ける表現を増やす」路線から一度離れ、dogfooding で見つかった**「黙って効かない」「検証されない」「CI から見えない」**穴を塞ぐ回にする。Phase は価値と実装コスト・依存関係の順に並べている（着手時に個別に設計する）。

| Phase | 内容 | 状態 |
|---|---|---|
| **46 診断の機械可読出力** | `yuzu check` / `lint` の出力は現在 `path:line:col: severity[rule] メッセージ` のテキスト 1 本だけで、CI は**終了コードしか見ていない**（違反があっても PR には「ジョブが赤い」としか出ない）。`--format {human,json,github}` を足し、GitHub Actions アノテーション形式で **PR の diff 行へ直接出す**。`yuzu search --json` という前例があるので CLI の慣習にも沿う（`Diagnostic` に `Serialize` を足すだけ。`SourceSpan` は既に derive 済み）。あわせて **lint ルール ID のリファレンス表**を docs へ（実装済み 16 ルール中 **13 が docs に一度も登場しない**。出力には `warning[alias-conflict]` と ID が出るのに引く先が無い） | ⬜ |
| **47 検証の網羅性** | **`openapi` / `jsonschema` の `file:` 参照が `yuzu check` で検証されない**。コンテンツインクルード（`file=`）は `include-error` で拾うのに、仕様ファイルを消す・改名する・YAML を壊すと check は 0 を返し CI も通り、公開サイトにエラーボックスだけが出る（docs サイト自身が `file: specs/sample-api.yaml` を使っている）。同じ「プロジェクトルート相対のファイル参照」なので `validate_includes` と同型に揃える。あわせて **`yuzu.jsonc` のスキーマ診断**: 現在エラーに行・列もキー名も出ず（`serde_json::from_value` 経由のため）、キーのタイポは**無言で無視される**。frontmatter には `frontmatter-unknown-key` があるのに設定側に無い非対称を解消する | ⬜ |
| **48 watch・キャッシュの正しさ** | Phase 42（コンテンツインクルード）がプロジェクトルート監視へ広げた副作用の解消。①**検索インデックスのキャッシュがページ source ハッシュだけで判定する**ため、インクルード参照先だけを編集すると本文は更新されるのに**検索結果が古いまま**になる（本文 HTML は external_deps で非対象化済みなのに検索 tf は対象外）②監視除外が出力ディレクトリと隠しディレクトリだけで、**`target/` や `node_modules/` を丸ごと再帰監視**する（`dev.watchIgnore` を設定可能に）③**`yuzu dev` が `yuzu.jsonc` の変更で再ビルド・ライブリロードするのに設定は反映しない**（起動時の設定で固定）ため「効かない → 設定ミスを疑う」で時間を溶かす。最低でも警告、可能なら再読込 | ⬜ |
| **49 dogfooding 改善** | 恒例のバッファ枠（着手時にユーザが 3 点選ぶ）。候補: **印刷用 CSS**（`@media print` が皆無で、設計書を PDF 保存するとサイドバー・TOC・検索ボックス・コピーボタンが全部紙に載る）/ **検索ドロップダウンの上位 10 件打ち止め**（「全 N 件」と出るのに残りを見る手段が無い。CLI には `--limit` があるのにブラウザ側に無い）/ `yuzu fmt --diff`（現在は「整形差分があります」だけで差分箇所が出ない）/ ポート衝突時のエラーにポート番号を出す / `--root` グローバルオプション / shell 補完（clap_complete）/ **scaffold の `index.md` が「5 図種」のまま**（実際は 9。`yuzu new` したユーザが最初に見るページ） | ⬜ |

v0.11 以降の候補: タブ / コードグループ（言語別サンプル・OS 別手順の切替。**comrak 0.53 の `block_directive` で `:::tabs` を自前パーサなしに実現できる**ことが判明。フェンスに `tab="Rust"` を足す案なら素の Markdown ビューアでも無傷）・i18n（テーマ UI 文字列の多言語化。`site.lang` は現在 `<html lang>` にしか効いていない。コア UI で 36 文字列）・全文検索の結果専用ページ・lint の inline 抑制と全ルールの enable/disable・外部リンク切れ検査（「決定的・オフライン」の凍結方針と衝突するため opt-in 設計が要る）・ドキュメントバージョニング（要否含め保留中）・VS Code 拡張（wasm プレビュー。`yuzu-core` / `yuzu-render` が 9 ファイルで `std::fs` に依存しており I/O 抽象化が前提）・yuzu 本体の crates.io 公開（汎用ライブラリ層は tankan・mikan まで公開済み。残るは本体だが、名前 `yuzu`・`yuzu-core` が別プロジェクトに取得済みのため、単一パッケージ化するか名称を再検討する必要がある。Phase 37 の決定事項）。
...
| Phase | 内容 | 状態 |
|---|---|---|
| **36 yuzu 自身のドキュメントサイト公開** | dogfooding の総仕上げとして、yuzu 自身のドキュメントを yuzu で書いて GitHub Pages に公開（https://ai-implementer.github.io/yuzu/ ）。`docs/` をこのリポジトリ自身の yuzu プロジェクトにし（`docs/yuzu.jsonc` ＋ content 16 ページ: トップ＋ガイド 9・リファレンス 3・開発 3）、README をページ階層へ再構成。主要機能を実運用で使用: tankan SSR（9 図種ギャラリー＋ワークスペース依存図。フォールバック 0 を CI でゲート）・OpenAPI（インライン＋ `file: specs/sample-api.yaml` 参照）・数式・検索（`indexCode` / `synonyms` / `lint.terms` クエリ拡張を有効化）・`lint.maxDirectoryDepth`・git 連携メタ（`fetch-depth: 0` で lastUpdated）。デプロイは `.github/workflows/docs.yml`（main push で自前 release バイナリをビルド → `yuzu check` 品質ゲート → `--base-url` に Pages のフル URL を注入 = llms.txt が絶対 URL → Pages へ配置）。ci.yml にも docs の check・build・SSR フォールバック検出を追加し、壊れた原稿はマージ前に検出する | ✅ |
