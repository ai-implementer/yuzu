...
- 生成日: 2026-07-20（wasm-bindgen 0.2.126 / binaryen version_130 / wasm-opt -Oz。
  crate 改名 yuzu-search-wasm→mikan-wasm・yuzu-index-format→mikan に伴う再 vendor。
  差分は wasm の name section のみ（40 バイト縮小。エクスポート API・search.js・
  JS クライアントは不変）。インデックスフォーマット v3 = postings の出現位置＋
  `"..."` フレーズ照合＋近接ブースト対応。manifest に `contentHash`・OPFS キャッシュ層同梱）
- `search_bg.wasm`: 494KB（vaporetto + fst + BM25 エンジン + フレーズ隣接照合 +
  ...
  ＝ `mikan-wasm` の Cargo 依存・エクスポート API は変えていない）
