...
fn index_code_でコード内の関数名がヒットし抜粋に出る() {
    ...
    build_search_index(&site, &md_opts, &IndexParams::default(), dist_off.path()).unwrap();
    assert!(
    ...
    build_search_index(&site, &md_opts, &params, dist_on.path()).unwrap();
    ...
    // 特別レンダリング対象（mermaid ソース）は index_code=true でも索引しない
    assert!(
        search_dist(dist_on.path(), "flowchart", 10)
            .unwrap()
            .is_empty(),
        "mermaid ソースは索引されない"
    );
