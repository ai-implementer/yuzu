...
pub(crate) fn parse(source: &str) -> Result<ErDiagram, Error> {
    ...
    for (idx, raw) in source.lines().enumerate() {
        ...
        if matches!(keyword, "style" | "classDef" | "class") || line.contains(":::") {
        ...
        if let Some(rest) = line.strip_suffix('{') {
            ...
            if !rest.is_empty() && !rest.contains("--") && !rest.contains("..") {
                let (name, display) = parse_entity_name(rest, line_no)?;
                attr_block = Some(p.intern(&name, display));
                continue;
            }
        }
        ...
        if find_relation_token(line).is_some() {
            ...
            let (name, display) = parse_entity_name(line, line_no)?;
            p.intern(&name, display);
        }
    }
...
impl ErParser {
    ...
    fn relation(&mut self, line: &str, line_no: usize) -> Result<(), Error> {
        ...
        let (from_name, from_disp) = parse_entity_name(from_str, line_no)?;
        let (to_name, to_disp) = parse_entity_name(to_str, line_no)?;
        let from = self.intern(&from_name, from_disp);
        let to = self.intern(&to_name, to_disp);
...
mod tests {
    ...
    fn スタイル系は_unsupported() {
        for src in [
            ...
            assert!(parse(&er(src)).unwrap_err().is_unsupported(), "{src}");
        }
    }
