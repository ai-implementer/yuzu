...
恒例のバッファ枠。候補からユーザが 5 点を選定した（他は次の枠・v0.12 へ）。

- **`cjk_friendly_emphasis`** — `**「重要」**です` のように日本語の約物に隣接した強調が
  CommonMark の flanking 規則で閉じられず、書き手が語順を変えて回避していた。
  comrak のフラグ 1 つ。既存の生成物は実測で差分ゼロ（docs / scaffold / fixtures を
  開き側・閉じ側の両パターンで走査して該当なし）
- **定義リスト（`description_lists`）** — 用語 → 空行 → コロンで始まる段落が `<dl>`。
  **用語集ページ（Phase 52）の生成形は据え置く** — `<dt>` は id を持たないので、
  `<dl>` 化するとアンカー `#ssr`・検索のセクション分割・右 TOC・linkcheck・CI ゲートが
  同時に壊れる。「本文の流れで説明する」用途に限る
- **検索結果の絞り込み（エンジン側）** — 下記
- **ポート衝突の案内 ＋ `build --watch` の `--port` / `--host`** — `dev` を 2 つ立てると
  「Address already in use」だけが出てポート番号も逃げ道も分からなかった。
  `build --watch` はサーバを起こすのに `dev.port` 固定で並走できなかった
- **キャッシュ保存の原子化** — `global.json`（事実上のコミットレコード）だけ tmp + rename

`CACHE_FORMAT_VERSION` 18 → 19。落とした候補と理由: **syntect.css の条件出力**は
`base.jinja` を上書きしている利用者のサイトでハイライトが効かなくなる非互換で、
やるならテーマ契約の変更として設計する（下の持ち越しに残す）。**印刷用 CSS** と
**`--root` / shell 補完**は次の枠へ。

#### 検索結果の絞り込み（設計判断）

絞り込みの区分は**ナビ第 1 階層**にした。`url` の先頭セグメントをそのまま出すと
`guide` / `development` の生ディレクトリ名になり日本語サイトで読めない。ナビなら
表示名は `<dir>/index.md` の title、並びも `order` に従うので**チップの並びが
サイドバーと一致する**（docs では実際にパス順とナビ順が食い違う）。

- **`FORMAT_VERSION` は 3 のまま**。`docGroups` / `groups` は `synonyms` /
  `content_hash` と同じ `serde(default)` の後方互換フィールドで、インデックスの
  バイナリ表現を変えない。**上げると害がある** — manifest.json は毎回フェッチされるのに
  `search_bg.wasm` は固定 URL で HTTP キャッシュに残るため、再デプロイ直後の再訪問者が
  「新 manifest ＋ 旧 wasm」に入り `VersionMismatch` で検索が全停止する。据え置けば
  「絞り込みが出ないだけ」の縮退で済む。`content_hash` の対象（fst ＋ シャード ＋
  モデル）も変わらないので OPFS キャッシュは無効化されない
- 同じ理由で **wasm 側は既存 `search(query, limit)` のシグネチャを変えず
  `searchIn` を新設**した。引数を足す設計だと、旧 wasm が余分な引数を黙って無視して
  「絞り込んでいないのに絞り込んだ件数」を出す（静かに嘘をつく）
- **フィルタは BM25 スコアリングの後**に `retain`（フレーズ filter と同じ位置）。
  前段で落とすと idf がグループ内 df になって絞り込みの有無で順位が入れ替わり、
  ファセット件数（絞り込み**前**の値）も取れない
- mikan は公開 API の追加メソッドで拡張したが、`Manifest` / `DocumentInput` /
  `BuildOptions` へのフィールド追加が破壊的変更なので **0.1.0 → 0.2.0**
- UI のチップ行は **listbox の外**（`header.jinja` は触らない = テーマを上書きしている
  利用者が静かに機能を失わない）。選択は sessionStorage に**表示名で**保存して
  ページ遷移をまたいで保持し、検索結果から遷移したときだけ一覧を開き直す

**副産物**: 外側クリックの判定に `ev.target.closest()` を使っていたため、
ハンドラ内の再描画で押した要素が DOM から外れると必ず null になり、検索が閉じていた。
絞り込みチップだけでなく**「さらに N 件を表示」も踏んでいた**既存の不具合で、
判定を `composedPath` へ変えて両方を直した。
