...
（yuzu-index-format）＋同一のモデルバイト**を使います。分かち書きが
1 トークンでもずれると検索は静かに壊れるため、抜粋生成・ハイライトの
ロジックも yuzu-index-format に 1 実装だけ置き、native / wasm で共有します。
...
| 資産 | 置き場所 | 更新スクリプト |
| --- | --- | --- |
| vaporetto モデル | `crates/yuzu-index-format/assets/model/` | `scripts/vendor-vaporetto-model.sh` |
