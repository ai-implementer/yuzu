...
改行・コメント・末尾カンマ、秒を省略した時刻の 3 つです。

この 3 つは公式 toml-test で「TOML 1.0 では invalid、1.1 では valid」と
裏付けられたものだけを入れています（`files-toml-1.0.0` の invalid 一覧にあり、
`files-toml-1.1.0` の invalid 一覧に無いケース）。**引用符なしのキーは 1.1 でも
`A-Za-z0-9_-` に限られる**ので、`サーバ = 1` はここには入らず、引用を促す普通の
キー構文エラーになります。
