...
- **合成ページ（`Page.generated` = 用語集）は「リンク先としてだけ」有効にする**。`build_site_model` と `build_source_pages` の両方に混ぜてあるので nav・検索・sitemap・llms・route 衝突検査・linkcheck のターゲットは無料で効くが、**`page.src` は実在しない**。`fs::write(&page.src)` する fmt / lint --fix は必ず除外する（`fs::write` は新規作成するので、忘れると `yuzu fmt` が `content/glossary.md` を実体化する）。lint・整形差分・診断のリンク元・集計行のページ数も同様
