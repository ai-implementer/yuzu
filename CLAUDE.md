...
### tankan の crates.io 公開（yuzu のリリースと非同期）

tankan だけは crates.io へ公開している（monorepo のまま。バージョンは workspace と独立で、`crates/tankan/Cargo.toml` の `version` を明示指定）。tankan に変更が溜まったら: version を上げる → `cargo build`（Cargo.lock 追随）→ CI green → `cargo publish --dry-run -p tankan` → `cargo publish -p tankan`（要 `cargo login`。公開は取り消し不可・yank のみ可能）。ci.yml の `cargo package --locked -p tankan` がメタデータ・同梱内容の回帰を PR で検出する。

tankan 以外の crate は crates.io に公開しない（`publish = false`。名前 `yuzu`・`yuzu-core` が別プロジェクトに取得済みのため。将来 検索スタック（yuzu-index-format）→ 本体の順で公開する構想は README ロードマップ参照）。
